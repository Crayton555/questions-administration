...
const CategoryQuestionDragDrop = () => {
    ...
    const [categories, setCategories] = useState([]); // Declare the categories state
    useEffect(() => {
        Promise.all([
            QuestionsAdministrationService.fetchCategories(),
            QuestionsAdministrationService.fetchQuestions()
        ]).then(([categoriesResponse, questionsResponse]) => {
        ...
        <DragDropContext onDragEnd={onDragEnd}>
            <div style={{ display: 'flex', flexWrap: 'wrap' }}>
                {categories.map((category, index) => {
                    ...
                    return (
                        <Droppable droppableId={String(category?.id)} key={category?.id}>
                            {(provided) => (
                                <div
                                ...
                                >
                                    <h3 style={{ background: '#4CAF50', color: 'white', padding: '10px' }}>{category?.name}</h3>
                                    {category?.questions?.map((question, index) => {
                                        ...
                                        return (
                                            <Draggable key={question.id} draggableId={String(question.id)}
                                                       index={index}>
                                                {(provided) => (
                                                    <div
    ...
    //                                 ref={provided.innerRef}
    //                                 {...provided.droppableProps}
    //                                 style={{ width: '50%', padding: '10px', boxSizing: 'border-box' }} // Category container
    //                             >
    //                                 <h3 style={{ background: '#4CAF50', color: 'white', padding: '10px' }}>{category?.name}</h3> {/* Category name with background */}
    //                                 {category?.questions?.map((question, index) => {
    //                                     if (!question) {
    //                                         console.error("Undefined question found in category:", category);
    //                                         return null;
    ...
    //                                                     <div style={{ background: '#f0f0f0', margin: '5px 0', padding: '10px', borderRadius: '5px' }}>
    //                                                         {question.name}
    //                                                     </div>
    //                                                 </div>
    //                                             )}
